...
const ObjectRepository: React.FC<ObjectRepositoryProps> = ({ activeProject, onAlert }) => {
    ...
    const loadObjects = async () => {
        setLoading(true);
        try {
            const data = await assetsApi.getObjects(activeProject.id);
            setObjects(data);
        } catch (e) {
            console.error(e);
    ...
    const handleSave = async () => {
        try {
            if (currentObject.id) {
                ...
                await assetsApi.createObject({ ...currentObject, project_id: activeProject.id, is_active: true });
    ...
    return (
        <div className="h-full flex flex-col">
            <div className="flex justify-between items-center mb-6">
                ...
                <button
                    onClick={() => { setCurrentObject({ selector_type: 'css' }); setIsEditing(true); }}
            ...
            <div className="flex-1 overflow-auto bg-white dark:bg-[#16191f] border border-gray-200 dark:border-gray-800 rounded-xl shadow-sm">
                <table className="w-full text-left">
                    <thead className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800">
                        <tr>
                            <th className="px-6 py-3 text-xs font-black text-gray-500 uppercase tracking-wider">Name</th>
                            <th className="px-6 py-3 text-xs font-black text-gray-500 uppercase tracking-wider">Type</th>
                            <th className="px-6 py-3 text-xs font-black text-gray-500 uppercase tracking-wider">Value</th>
                            <th className="px-6 py-3 text-xs font-black text-gray-500 uppercase tracking-wider">Actions</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                        {filteredObjects.map(obj => (
                            <tr key={obj.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
                                ...
                                <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400 font-mono truncate max-w-xs">{obj.value}</td>
