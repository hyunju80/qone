...
interface PersonaManagerProps {
  ...
  activeProjectDomain?: string; // 추가: 현재 프로젝트의 도메인
  onAddPersona: (persona: Persona) => void;
  onUpdatePersona: (persona: Persona) => void;
}

const PersonaManager: React.FC<PersonaManagerProps> = ({ personas, activeProjectId, activeProjectDomain, onAddPersona, onUpdatePersona }) => {
  ...
  const [newPersona, setNewPersona] = useState<Partial<Persona>>({
    ...
    type: 'USER',
    domain: activeProjectDomain || 'General',
    motivation: '',
    currentState: ''
  });
  ...
  const handleSavePersona = () => {
    ...
    const persona: Persona = {
      ...
      projectId: newPersona.projectId || activeProjectId,
  ...
  const filteredPersonas = personas
    .filter(p => {
      ...
      // 2. 도메인 필터링 (Telecom 프로젝트에서 Finance가 나오지 않도록)
      // SYSTEM 페르소나이거나 projectId가 'global'인 경우에만 도메인 필터링 적용
      if ((p.type === 'SYSTEM' || p.projectId === 'global') && activeProjectDomain && p.domain) {
        const pDomain = p.domain.toLowerCase();
        const activeDomain = activeProjectDomain.toLowerCase();
        // 현재 도메인과 일치하거나 'General'인 것만 반환
        return pDomain === activeDomain || pDomain === 'general';
      }
    ...
    .sort((a, b) => {
      ...
      // 2. 도메인 일치 우선
      const aMatch = aDomain === activeDomain;
      const bMatch = bDomain === activeDomain;
  ...
  const PREDEFINED_TRAITS = [
    'Careful', 'Bold', 'Lazy', 'Impatient', 'Analytical',
    'Persistent', 'Trustful', 'Skeptical', 'Frustrated'
  ];
    ...
    <div className="h-full flex flex-col">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-4">
          <div className="relative w-64">
            ...
            <input
              ...
              className="w-full bg-white dark:bg-[#16191f] border border-gray-200 dark:border-gray-800 rounded-lg pl-10 pr-4 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
            />
          </div>
          <div className="text-gray-500 dark:text-gray-400 text-xs italic font-medium">
        ...
        <button
          ...
          className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-bold flex items-center gap-2 shadow-lg shadow-indigo-600/20"
        >
      ...
      <div className="flex-1 overflow-auto pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPersonas.map(p => {
            const isDomainMatch = p.domain?.toLowerCase() === activeProjectDomain?.toLowerCase();
            const isSystem = p.type === 'SYSTEM';

            return (
              <div
                key={p.id}
                className={`bg-white dark:bg-[#16191f] border rounded-2xl p-5 hover:border-indigo-500 hover:shadow-lg transition-all flex flex-col group relative overflow-hidden h-auto ${!p.isActive
                  ? 'border-red-200 dark:border-red-900/30 opacity-60 grayscale'
              ...
              >
                ...
                <div className="flex gap-6 mb-6">
                  <div className={`w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0 transition-all shadow-inner ${!p.isActive
                      ? 'bg-gray-100 dark:bg-gray-900 text-gray-400 dark:text-gray-700'
                      : isSystem
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2">
                          ...
                          {p.type === 'SYSTEM' ? (
                            <span className="flex items-center gap-1 px-1.5 py-0.5 rounded bg-amber-50 dark:bg-amber-600/10 border border-amber-200 dark:border-amber-500/30 text-[8px] font-black text-amber-600 dark:text-amber-400 uppercase tracking-widest shadow-sm">
                              <Users className="w-2.5 h-2.5" /> Qone System
                            </span>
                          ) : p.projectId === 'global' ? (
                        ...
                        {p.domain && (
                          <span className={`text-[9px] font-bold uppercase tracking-tighter ${p.domain === activeProjectDomain ? 'text-indigo-500' : 'text-gray-400'}`}>
                            Domain: {p.domain}
                          </span>
                        )}
                      </div>
                      <button
                        ...
                        disabled={p.type === 'SYSTEM'}
                        className={`p-1 rounded transition-colors relative z-10 ${p.type === 'SYSTEM' ? 'opacity-30 cursor-not-allowed' : 'hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                        title={p.type === 'SYSTEM' ? 'System personas cannot be deactivated' : p.isActive ? 'Deactivate' : 'Activate'}
                      >
                ...
                <div className="flex-1 space-y-4">
                  {p.motivation && (
                    <div className="space-y-1">
                      <div className="text-[8px] font-black text-indigo-500/60 uppercase tracking-widest flex items-center gap-1">
                        <Target className="w-2.5 h-2.5" /> Motivation
                      </div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {p.traits.map((trait, idx) => (
                      <span key={idx} className="bg-gray-50 dark:bg-indigo-500/5 text-gray-600 dark:text-indigo-400/80 text-[9px] font-bold px-2 py-0.5 rounded-md border border-gray-200 dark:border-indigo-500/10">
                        {trait}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="flex gap-2">
                    {p.type === 'SYSTEM' ? (
                      <button
                        ...
                        className="text-[10px] font-black text-amber-600 dark:text-amber-400 hover:text-amber-500 dark:hover:text-amber-300 uppercase tracking-widest flex items-center gap-1 bg-amber-50 dark:bg-amber-500/10 px-3 py-1.5 rounded-lg border border-amber-100 dark:border-amber-500/20 transition-colors"
                      >
                        <Plus className="w-3 h-3" /> Use as Template
                      </button>
                    ) : (
                      <button
            ...
            <div className="relative w-full max-w-2xl bg-white dark:bg-[#16191f] border border-gray-200 dark:border-gray-800 rounded-3xl shadow-2xl flex flex-col overflow-hidden max-h-[90vh] transition-colors">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-[0.2em] transition-colors">
                      {editingPersonaId ? 'Modify Persona Context' :
                        newPersona.type === 'SYSTEM' ? 'Use System Template' : 'Create Behavioral Persona'}
                    </h3>
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Domain</label>
                    <select
                      value={newPersona.domain}
                      onChange={(e) => setNewPersona({ ...newPersona, domain: e.target.value })}
                      className="w-full bg-gray-50 dark:bg-[#0c0e12] border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white focus:border-indigo-500 outline-none transition-all"
                    >
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Motivation (Why?)</label>
                    <input
                      type="text"
                      value={newPersona.motivation}
                      onChange={(e) => setNewPersona({ ...newPersona, motivation: e.target.value })}
                      placeholder="e.g., To check balance security"
                      className="w-full bg-gray-50 dark:bg-[#0c0e12] border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white focus:border-indigo-500 outline-none transition-all placeholder:text-gray-400"
                    />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Current State (Inventory/Context)</label>
                  <input
                    type="text"
                    value={newPersona.currentState}
                    onChange={(e) => setNewPersona({ ...newPersona, currentState: e.target.value })}
                    placeholder="e.g., 5 items in cart, 1 expired coupon"
                    className="w-full bg-gray-50 dark:bg-[#0c0e12] border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white focus:border-indigo-500 outline-none transition-all placeholder:text-gray-400"
                  />
                </div>
                ...
                <div className="space-y-4">
                  <label className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Behavioral Traits (Selectable Tags)</label>
                  <div className="flex flex-wrap gap-2 p-4 bg-gray-50 dark:bg-[#0c0e12] border border-gray-200 dark:border-gray-800 rounded-2xl shadow-inner transition-colors">
                    {PREDEFINED_TRAITS.map((trait) => {
                      const isSelected = newPersona.traits?.includes(trait);
                      return (
                        <button
                          key={trait}
                          onClick={() => {
                            ...
                            if (isSelected) {
                              setNewPersona({ ...newPersona, traits: currentTraits.filter(t => t !== trait) });
                            } else {
                              setNewPersona({ ...newPersona, traits: [...currentTraits, trait] });
                            }
                          }}
                          className={`px-3 py-1.5 rounded-full text-[10px] font-bold uppercase transition-all border ${isSelected
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-gray-50 dark:bg-[#0c0e12] border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white focus:border-indigo-500 outline-none transition-all placeholder:text-gray-400"
                    />
                    <button
                      onClick={handleAddTrait}
                      className="bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600 dark:text-white px-4 rounded-xl flex items-center justify-center border border-gray-200 dark:border-gray-700"
                    >
