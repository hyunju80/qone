...
const Settings: React.FC<SettingsProps> = ({
  ...
  const [categoryForm, setCategoryForm] = useState<Partial<CategoryNode>>({ name: '', description: '', parentId: '', managerId: '' });
  ...
  // --- Category Handlers ---
  const handleOpenCategoryModal = (catToEdit: CategoryNode | null = null) => {
    setEditingCategory(catToEdit);
    setCategoryForm(catToEdit ? { ...catToEdit } : { name: '', description: '', parentId: '', managerId: '' });
    setIsCategoryModalOpen(true);
  };

  const handleSaveCategory = async () => {
    if (!activeProject || !categoryForm.name?.trim()) return;

    const currentCategories = activeProject.categories || [];
    let updatedCategories = [...currentCategories];

    if (editingCategory) {
      updatedCategories = updatedCategories.map(c =>
        c.id === editingCategory.id ? { ...c, ...categoryForm } as CategoryNode : c
      );
    } else {
      // Check for duplicate names
      if (currentCategories.some(c => c.name.toLowerCase() === categoryForm.name!.trim().toLowerCase())) {
        onAlert("Warning", "A category with this name already exists.", 'error');
        return;
      }
      const newCategory: CategoryNode = {
        ...
        managerId: categoryForm.managerId || null
      };
    ...
    try {
      await projectsApi.update(activeProject.id, { categories: updatedCategories });
      onUpdateProjects(projects.map(p => p.id === activeProject.id ? updatedProject : p));
      setIsCategoryModalOpen(false);
      onAlert("Success", "Category saved successfully.", 'success');
    } catch (e) {
      onAlert("Error", "Failed to save category.", 'error');
    }
  };
  ...
  const handleDeleteCategory = async (catId: string) => {
    ...
    // Optional: Warn if it has children?
    const updatedCategories = (activeProject.categories || []).filter(c => c.id !== catId);

    // Also remove parent references to the deleted category
    const scrubbedCategories = updatedCategories.map(c =>
      c.parentId === catId ? { ...c, parentId: null } : c
    );
    ...
    try {
      await projectsApi.update(activeProject.id, { categories: scrubbedCategories });
      onUpdateProjects(projects.map(p => p.id === activeProject.id ? updatedProject : p));
      onAlert("Success", "Category deleted.", 'success');
    } catch (e) {
      onAlert("Error", "Failed to delete category.", 'error');
    }
  };
    ...
    <div className="p-8 max-w-7xl mx-auto h-full overflow-y-auto custom-scrollbar bg-gray-50 dark:bg-[#0c0e12] transition-colors">
      ...
      {isCategoryModalOpen && createPortal(
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-8 bg-black/60 dark:bg-black/80 backdrop-blur-md animate-in fade-in duration-200 transition-colors">
          <div className="relative w-full max-w-md bg-white dark:bg-[#16191f] border border-gray-200 dark:border-gray-800 rounded-3xl p-8 shadow-2xl animate-in zoom-in-95 transition-colors">
            <h3 className="text-xl font-black text-gray-900 dark:text-white mb-6 uppercase tracking-widest flex items-center gap-2 transition-colors">
              <Network className="w-5 h-5 text-indigo-500 dark:text-indigo-400" /> {editingCategory ? 'Rename Category' : 'Define Category'}
            </h3>
            <div className="space-y-5 mb-8">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest transition-colors mb-1.5 block">Category Name</label>
                <input
                  autoFocus
                  value={categoryForm.name}
                  onChange={e => setCategoryForm({ ...categoryForm, name: e.target.value })}
                  placeholder="e.g., Auth, Payments, Search"
                  className="w-full bg-gray-50 dark:bg-[#0c0e12] border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white focus:border-indigo-500 outline-none transition-colors shadow-sm dark:shadow-none"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest transition-colors mb-1.5 block">Description</label>
                <input
                  value={categoryForm.description}
                  onChange={e => setCategoryForm({ ...categoryForm, description: e.target.value })}
                  placeholder="e.g., Handles all user authentication flows"
                  className="w-full bg-gray-50 dark:bg-[#0c0e12] border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-gray-300 focus:border-indigo-500 outline-none transition-colors shadow-sm dark:shadow-none"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest transition-colors mb-1.5 block">Parent Category</label>
                <select
                  value={categoryForm.parentId || ''}
                  onChange={e => setCategoryForm({ ...categoryForm, parentId: e.target.value || null })}
                  className="w-full bg-gray-50 dark:bg-[#0c0e12] border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-3 text-sm text-gray-700 dark:text-gray-300 outline-none focus:border-indigo-500 transition-colors shadow-sm"
                >
                  <option value="">-- Root Level --</option>
                  {(activeProject?.categories || []).filter(c => c.id !== editingCategory?.id).map(c => (
                    <option key={c.id} value={c.id}>{c.name}</option>
                  ))}
                </select>
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest transition-colors mb-1.5 block">Category Owner (Manager)</label>
                <select
                  value={categoryForm.managerId || ''}
                  onChange={e => setCategoryForm({ ...categoryForm, managerId: e.target.value || null })}
                  className="w-full bg-gray-50 dark:bg-[#0c0e12] border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-3 text-sm text-gray-700 dark:text-gray-300 outline-none focus:border-indigo-500 transition-colors shadow-sm"
                >
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-800">
              <button onClick={() => setIsCategoryModalOpen(false)} className="px-6 py-3 text-xs font-bold text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 transition-colors">Cancel</button>
              <button
