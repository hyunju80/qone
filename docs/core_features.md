...
## 3. AI Exploration (자율 탐색 및 검증)

AI Exploration은 사용자의 개입 없이 AI 에이전트가 스스로 판단하고 행동하여 테스트 목적을 달성하는 Q-ONE의 핵심 지능형 모듈입니다. '의도(Goal)'만 주어지면 복잡한 화면 구조를 분석하여 최적의 경로를 찾아내며, 수행된 과정은 즉시 재사용 가능한 테스트 자산으로 전환됩니다.

### 2.1. Autonomous Self-Driving Loop (자율 실행 루프)
*   **지능형 의사결정**: Gemini LLM을 통해 HTML/XML 구조를 분석하고, 목표(Goal) 달성을 위한 다음 액션을 스스로 결정합니다.
*   **Analyze-Act-Verify**: 액션 수행 전후의 상태를 비교하여 성공 여부를 스스로 판단하며, 장애물 발생 시 우회 경로를 탐색합니다.
*   **Multi-Turn History**: 세션 내의 모든 수행 이력을 기억하여 중복 동작을 방지하고, 맥락에 맞는 연속적인 테스트를 수행합니다.

### 2.2. Multi-Platform Intelligence (멀티 플랫폼 지원)
*   **Web & Mobile App**: Playwright(Web)와 Appium(Android)을 통합 제어하여 플랫폼에 구속되지 않는 범용적인 자율 탐색 환경을 제공합니다.
*   **Context-Aware Selectors**: 플랫폼별(CSS Selector, Accessibility ID, XPath) 최적화된 요소를 자동으로 식별하여 정밀하게 상호작용합니다.

### 2.3. Persona & Behavioral Testing (페르소나 기반 행동 테스팅)
*   **가상 인격체 주입**: Design Center에서 정의된 페르소나(초보자, 전문가, 특정 성향 등)를 탑재하여 해당 역할에 맞는 행동 양식으로 테스트를 진행합니다.
*   **Human-like Reasoning**: 단순한 기술적 검증을 넘어 페르소나의 관점에서 "왜 이 동작을 수행하는지"에 대한 사고 과정(Thought)을 기록합니다.

### 2.4. Landmark-Based Self-Verification (랜드마크 기반 자가 검증)
*   **Predictive Assertion**: 액션 수행 후 나타날 것으로 예상되는 텍스트(`expected_text`)를 미리 예측합니다.
*   **Actual Verification**: 다음 단계에서 실제로 발견된 텍스트(`actual_observed_text`)를 대조하여, 이전 단계의 액션이 실질적으로 성공했는지(예: "로그인 완료" 랜드마크 확인)를 확증합니다.

### 2.5. Human-in-the-Loop (하이브리드 제어)
*   **Interactive Feedback**: 에이전트가 탐색 중 판단이 모호하거나 로그인이 필요한 경우, 사용자가 실시간으로 피드백을 주어 올바른 방향으로 가이드할 수 있습니다.
*   **Manual Override (Edit & Retry)**: AI가 결정한 특정 스텝을 사용자가 즉시 수정하거나 삭제할 수 있으며, 수정된 시점부터 다시 자율 탐색을 재개할 수 있습니다.

### 2.6. Instant Assetization (즉시 자산화)
*   **Shift-Left Automation**: 성공적으로 완료된 탐색 세션은 별도의 스크립트 작성 과정 없이 클릭 한 번으로 `TestScript` 자산으로 변환되어 Asset Library에 등록됩니다.
*   **Execution Policy Inheritance**: 탐색 시 사용된 타겟 URL, Goal, 페르소나 설정이 자산의 기본 실행 정책으로 자동 상속됩니다.

---

## 4. Step Flow (룰 기반 테스트 케이스 생성)
...
## 6. DataSet Studio (대용량 테스트 데이터 생성 및 관리)
...
## 8. Smart Scheduler (지능형 테스트 오케스트레이션)
...
## 10. Admin & User Management (관리자 및 사용자 관리)
...
## 11. Workspace Configuration (워크스페이스 설정 및 관리)
...
## 12. Execution Status (실행 현황 및 모니터링)
...
## 13. Analytics & Reports Dashboard (품질 지능 대시보드)
...
## 14. Implementation Status (구현 현황 요약)
...
| **AI Exploration** | 자율 탐색 루프 (WEB/APP) | ✅ | Gemini 2.0 기반 자율 주행 엔진 연동 |
| | 페르소나 및 피드백 루프 | ✅ | 가상 인격체 및 사용자 가이드 기능 지원 |
| | 실시간 자산화 (Convert) | ✅ | 탐색 결과를 TestScript로 즉시 변환 |
